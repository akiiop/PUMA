function [V,f,Fun_eval,angles,W]=MCBS_DecompositionV6(U,F,N,MC)
...
fun_mat = @(x)1-UnitaryFidelity(U,U_Fun(x,F,d,N));

angles = cell(1,MC);
Fun_eval = nan(1,MC);

for k = 1:MC
    angles{k} = simulannealbnd(fun_mat,zeros(1,(d-1)*(N+1)),-2*pi,2*pi);
    Fun_eval(k) = fun_mat(angles{k});
end
